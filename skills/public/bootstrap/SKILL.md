...
2. **Draft 自检（必须）**：在展示给用户之前，先用下面清单检查并修正混写问题（不要把错误草稿丢给用户确认）：
   - **SOUL.md**：只保留人格资产（气质/价值观/世界观/习惯/表达风格/关系边界）。不得出现“职责/主要任务/交付物/典型输入输出/质量标准/边界与禁区”等角色段落或标题。
   - **IDENTITY.md**：只保留角色资产（职责范围/交付物形态/质量标准/澄清策略/边界禁区）。不得出现“气质/世界观/价值观/表达癖好”等人格段落或标题。
   - **memory_items**：仅写稳定事实或偏好；3–8 条；每条 1 个信息点；tier 只允许 `profile`/`preference`；用户不愿写入则输出 `[]` 并落盘时不要传该参数。
3. 把草稿展示给用户确认。展示时要明确：SOUL/IDENTITY 会落盘并在运行时被注入系统提示词；memory_items 会写入 OpenViking Memory（因此要短、稳定、可复用）。
4. 允许用户对草稿做 1–2 轮微调，直到用户明确确认。
5. 用户确认后，调用 `setup_agent` 落盘（必须调用工具，不要手写文件）：
...
- **工具侧有兜底门禁。** 如果 SOUL.md 混入明显职责/交付物等段落，`setup_agent` 会拒绝写盘；你应在 Draft 自检阶段先修正。
