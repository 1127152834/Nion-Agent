...
> **⚠️ 前置依赖**: 本计划假设 `backend/` 已完成 harness/app 目录重构（见 `2026-03-16-backend-harness-app-restructure.md`）。
> - 所有 import 路径已按 `nion.*`（harness 层）和 `app.*`（app 层）映射
> - `scheduler/runner.py` 已引入 `mode_registry` 回调机制（Phase 1D），evolution/heartbeat 的执行逻辑已在 `app/gateway/app.py` lifespan 中注册
> - **Task 2 中往 runner 添加 `TaskMode.EVOLUTION` 执行分支时**，应通过 `nion.scheduler.mode_registry.register_mode_executor` 注册，而非在 runner 内直接 lazy import `app.evolution`（否则违反 harness→app 边界）
...
**Files:**
- Modify: `/Users/zhangtiancheng/Documents/项目/新项目/Nion-Agent/backend/app/evolution/analyzer.py`
- Modify: `/Users/zhangtiancheng/Documents/项目/新项目/Nion-Agent/backend/app/gateway/routers/evolution.py`
...
Modify `/Users/zhangtiancheng/Documents/项目/新项目/Nion-Agent/backend/app/evolution/analyzer.py`，把签名改为接收 `agent_name`，并在内部按 agent 读取 Heartbeat/Soul/Memory（仅修正明显错用参数与统计字段，不做复杂策略）：
```python
...
Modify `/Users/zhangtiancheng/Documents/项目/新项目/Nion-Agent/backend/app/gateway/routers/evolution.py`，在 `/run` 包一层错误 detail（服务内部仍会落盘 failed report）：
```python
...
git add backend/app/evolution/analyzer.py backend/app/gateway/routers/evolution.py backend/tests/test_evolution_router.py
...
**Files:**
- Modify: `/Users/zhangtiancheng/Documents/项目/新项目/Nion-Agent/backend/packages/harness/nion/scheduler/models.py`
- Modify: `/Users/zhangtiancheng/Documents/项目/新项目/Nion-Agent/backend/packages/harness/nion/scheduler/runner.py`
- Modify: `/Users/zhangtiancheng/Documents/项目/新项目/Nion-Agent/backend/app/gateway/routers/scheduler.py`
...
from nion.scheduler.models import ScheduledTask, TriggerConfig, TriggerType
...
```
- **不要在 runner 内直接 import `app.evolution`**（违反 harness→app 边界）。重构已引入 `mode_registry`，evolution executor 已在 `app/gateway/app.py` lifespan 中注册。只需确保 runner 的 `_execute_workflow()` 对 `TaskMode.EVOLUTION` 走 `get_mode_executor("evolution")` 调用路径（与 heartbeat 同理）。如果 `get_mode_executor` 返回 None，raise `ValueError`。
...
```bash
cd /Users/zhangtiancheng/Documents/项目/新项目/Nion-Agent.wt/evolution-heartbeat-closure-fix
git add backend/packages/harness/nion/scheduler/models.py backend/packages/harness/nion/scheduler/runner.py backend/app/gateway/routers/scheduler.py backend/tests/test_scheduler_evolution_mode.py
...
**Files:**
- Modify: `/Users/zhangtiancheng/Documents/项目/新项目/Nion-Agent/backend/app/evolution/service.py`
- Modify: `/Users/zhangtiancheng/Documents/项目/新项目/Nion-Agent/backend/app/evolution/models.py`
...
Modify `/Users/zhangtiancheng/Documents/项目/新项目/Nion-Agent/backend/app/evolution/models.py`（加约束，避免无效 interval）：
```python
...
Modify `/Users/zhangtiancheng/Documents/项目/新项目/Nion-Agent/backend/app/evolution/service.py`（核心逻辑）：
- 在 `update_settings()` 中：
...
```bash
cd /Users/zhangtiancheng/Documents/项目/新项目/Nion-Agent.wt/evolution-heartbeat-closure-fix
git add backend/app/evolution/models.py backend/app/evolution/service.py backend/tests/test_evolution_auto_trigger.py
...
**Files:**
- Modify: `/Users/zhangtiancheng/Documents/项目/新项目/Nion-Agent/backend/app/gateway/routers/heartbeat.py`
- Modify: `/Users/zhangtiancheng/Documents/项目/新项目/Nion-Agent/backend/app/heartbeat/service.py`
...
```bash
cd /Users/zhangtiancheng/Documents/项目/新项目/Nion-Agent.wt/evolution-heartbeat-closure-fix
git add backend/app/gateway/routers/heartbeat.py backend/app/heartbeat/service.py \
