...
// Spline scene URL for reading assistant character
const READING_ASSISTANT_SPLINE_URL = "https://assets.folo.is/ai2.splinecode";
...
export function AICharacter3D({ className }: AICharacter3DProps) {
  ...
  const handleLoad = useCallback(
    (app: SplineApp) => {
      // Find the reading assistant character object
      const head = app.findObjectByName("Reading Assistant Character");
      if (!head) {
        console.warn("Cannot find Reading Assistant Character object");
        return;
      }
      ...
      const onClick = () => {
        app.emitEvent("mouseDown", "Reading Assistant Character");
      };
      onClick();
  ...
  return (
    <div ref={containerRef} className={cn("h-48 w-full", className)}>
      <Spline
        scene={READING_ASSISTANT_SPLINE_URL}
        onLoad={handleLoad}
