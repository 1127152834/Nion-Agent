(function () {
  ...
  function updateButtons() {
    refs.buildBtn.textContent = state.build.succeeded ? "重新编译" : "编译";
    ...
    refs.startBtn.textContent = state.dev.running ? "停止" : "启动";
    ...
    apiCall("runCommand", {
      ...
      .then(function (result) {
        ...
        return streamCommandSession(result.sessionId, function (payload) {
          ...
          setStatus(state.build.succeeded ? "编译成功" : "编译失败");
          updateButtons();
        });
      })
      .catch(function (err) {
        ...
        setStatus("编译失败");
        updateButtons();
        toast("编译命令执行失败: " + (err && err.message ? err.message : err), "error");
      });
  }
    ...
    return resolveProjectRoot().then(function (projectRoot) {
      ...
      return detectPackageManager(projectRoot).then(function (pm) {
        ...
        .then(function (raw) {
          ...
          const rootLabel =
            projectRoot === ROOT ? "前端项目" : "前端项目 · " + projectRoot.slice(ROOT.length + 1);
          ...
          return apiCall("persistState.get", { key: "code-viewer.commands" })
        ...
        .catch(function () {
          state.scripts = {};
          refs.projectBadge.textContent = "普通目录";
          refs.buildCmd.value = "";
          refs.devCmd.value = "";
  ...
  function saveCommandPreferences() {
    ...
    apiCall("persistState.set", { key: "code-viewer.commands", value: payload }).catch(function () {
      return undefined;
    });
  }
